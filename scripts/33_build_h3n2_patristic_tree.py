#!/usr/bin/env python3
"""Build a FastTree patristic-distance tree for the H3N2 aligned HA sequences."""
...
def main() -> None:
    ...
    aligned_fasta = Path(cfg["output_dir"]) / f"{cfg['subtype']}_HA_aligned.fasta"
    if not aligned_fasta.exists():
        raise FileNotFoundError(f"Aligned HA FASTA not found: {aligned_fasta}")
    ...
    lines = [
        f"Aligned HA FASTA: {aligned_fasta}",
    ...
    append_qc_log(cfg, "33_build_h3n2_patristic_tree", lines)
    print("\n".join(lines))
